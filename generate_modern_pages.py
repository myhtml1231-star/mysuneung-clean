...
def main():
    ...
    # Some datasets (예: 2024-9, 2024-s, 2025-6, 2026-6, 2026-s) are
    # still waiting for their legacy "고3 등급컷" HTML sources. To keep
    # the modern UI consistent and surface the missing-data state, emit
    # placeholder pages so users aren't met with 404s.
    missing_slugs = {
        '2024-9': '2024학년도 9월 모의평가 등급컷',
        '2024-s': '2024학년도 수능 등급컷',
        '2025-6': '2025학년도 6월 모의평가 등급컷',
        '2026-6': '2026학년도 6월 모의평가 등급컷',
        '2026-s': '2026학년도 수능 등급컷',
    }
    ...
    placeholder_subject = [{
        ...
        'cards': [{
            'title': '데이터 준비중',
            'difficulty': '원본 HTML 대기',
            'grades': [
                {'grade': '1', 'desc': '알림', 'value': '기존 등급컷 HTML을 찾는 중입니다.'},
                {'grade': '2', 'desc': '안내', 'value': '소스가 확보되면 자동으로 업데이트됩니다.'},
                {'grade': '3', 'desc': '문의', 'value': '관리자에게 원본 파일을 추가해주세요.'},
            ],
        }],
    }]

    for slug, title in missing_slugs.items():
        if slug in produced:
            continue
        output = Path(f"{slug}-modern.html")
        output.write_text(render_page(title, placeholder_subject), encoding='utf-8')
